...
function SortableTreeRow({ row, focusedNodeId, selectedNodeIds, openMenuId, setOpenMenuId, setMenuPosition, renamingId, renameDraft, setRenameDraft, finishRename, cancelRename, hoveredGroupId, onToggleVisible, onSelectNode, onDoubleClickNode, onToggleGroupExpanded, registerRowElement, activeTool }: { row: FlatTreeRow; focusedNodeId: string | null; selectedNodeIds: string[]; openMenuId: string | null; setOpenMenuId: (id: string | null) => void; setMenuPosition: (position: MenuPosition | null) => void; renamingId: string | null; renameDraft: string; setRenameDraft: (value: string) => void; startRename: (row: FlatTreeRow) => void; finishRename: (nodeId: string) => void; cancelRename: () => void; hoveredGroupId: string | null; onToggleVisible: (nodeId: string) => void; onSelectNode: (nodeId: string, event: ReactMouseEvent<HTMLDivElement>) => void; onDoubleClickNode?: (nodeId: string) => void; onToggleGroupExpanded: (groupId: string) => void; registerRowElement?: (id: string, node: HTMLDivElement | null) => void; activeTool?: string; }) {
  ...
  const layerNode = !isGroup ? row.node as any : null;
  const isSliceBrowsableLayer = !!layerNode && (layerNode.type === "remote" || layerNode.type === "file") && layerNode.renderMode === "slices";
  const activePlane = isSliceBrowsableLayer ? (layerNode.axisSliceState?.activePlane ?? "xy") : null;
  ...
  return (
    ...
    >
      <RowShell row={row} selected={isSelected} inheritedSelected={inheritedSelected} hoveringGroupTarget={hoveringGroupTarget} dragStyle={style} dragging={isDragging}>
        <div onClick={(event) => onSelectNode(row.id, event)} onDoubleClick={() => onDoubleClickNode?.(row.id)} style={{ display: "flex", alignItems: "center", gap: 8 }}>
...
export default function LayerPanel({ layerTree, selectedNodeId, selectedNodeIdsExternal, activeTool: _activeTool, groupOptions: _groupOptions, detailsContent = null, isDetailsCollapsed = false, onToggleDetailsCollapsed, isCollapsed, onSetCollapsed, onToggleVisible, onSelectNode, onDoubleClickNode, onSelectionChange, onToggleGroupExpanded, onSetGroupExpanded, onAddGroup, onAddLayer, onRenameNode, onDuplicateNode, onDeleteNode, onDeleteNodes, onCreateGroupFromNodes, onDropNodeIntoGroup, onDropNodeToRoot, onReorderBefore, onDropNodesIntoGroup, onDropNodesToRoot, onReorderNodesBefore }: { layerTree: LayerTreeNode[]; selectedNodeId: string | null; selectedNodeIdsExternal?: string[] | null; activeTool?: string; groupOptions: LayerGroupNode[]; detailsContent?: React.ReactNode; isDetailsCollapsed?: boolean; onToggleDetailsCollapsed?: () => void; isCollapsed: boolean; onSetCollapsed: (collapsed: boolean) => void; onToggleVisible: (nodeId: string) => void; onSelectNode: (nodeId: string) => void; onDoubleClickNode?: (nodeId: string) => void; onSelectionChange?: (nodeIds: string[], preferredNodeId: string) => void; onToggleGroupExpanded: (groupId: string) => void; onSetGroupExpanded: (groupId: string, expanded: boolean) => void; onAddGroup: () => void; onAddLayer: () => void; onRenameNode: (nodeId: string, newName: string) => void; onDuplicateNode: (nodeId: string) => void; onDeleteNode: (nodeId: string) => void; onDeleteNodes: (nodeIds: string[]) => void; onCreateGroupFromNodes: (nodeIds: string[]) => void; onDropNodeIntoGroup: (nodeId: string, groupId: string) => void; onDropNodeToRoot: (nodeId: string) => void; onReorderBefore: (activeId: string, overId: string) => void; onDropNodesIntoGroup: (nodeIds: string[], groupId: string) => void; onDropNodesToRoot: (nodeIds: string[]) => void; onReorderNodesBefore: (nodeIds: string[], overId: string) => void; }) {
  ...
  function handleRowSelect(nodeId: string, event: ReactMouseEvent<HTMLDivElement>) {
    ...
    if (onSelectionChange) onSelectionChange(nextSelection, nodeId);
